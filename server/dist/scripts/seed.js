...
const buildEntries = (type, sections) => sections.flatMap(({ category, items }) => items.map(([title, marker = '']) => ({
    type,
    category,
    title,
...
const consolidatedMedia = Array.from(rawEntries.reduce((map, entry) => {
...
const run = async () => {
    ...
    await media_item_1.MediaItemModel.deleteMany({});
    await category_1.CategoryModel.deleteMany({});
    await category_group_1.CategoryGroupModel.deleteMany({});
    const categories = await category_1.CategoryModel.insertMany(categorySeeds);
    const categoryMap = new Map(categories.map((category) => [category.name, category._id]));
    await media_item_1.MediaItemModel.insertMany(consolidatedMedia.map((item) => ({
        title: item.title,
        type: item.type,
        status: item.status,
        rating: item.rating,
        ...
        categories: item.categoryNames.map((name) => categoryMap.get(name)).filter((value) => Boolean(value)),
    })));
